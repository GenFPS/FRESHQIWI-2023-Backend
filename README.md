...
# Данное задание было реализовано на Go

### Для запуска программы необходимо перейти в папку cmd, где расположен файл **main.go** и ввести в консоль/терминал следующую команду:

```
go run main.go --code=USD --date=2022-10-08
```

- где `code` и `date` являются параметрами ввода (валюты и даты, которые нас интересуют)
